...
app.config["SQLALCHEMY_DATABASE_URI"] = "postgresql://localhost/danimeyeHW5"
## Provided:
...
class UpdateItemPriority(FlaskForm):
    priority_level = StringField("Enter the new priority of this item: ", validators=[Required()])
    submit = SubmitField("Update")
...
def delete(lst):
    ...
    db.session.commit()
    flash("Successfully deleted {}".format(this_list.title))
    return redirect(url_for('all_lists'))
